	...
	if training:
		...
		for batch_data, batch_target in loader_train: # Unpack data and target
			...
			if 'TranAD' in model_name:
				...
				# Reshape batch_data to (batch_size, window_size, feats) before permuting
				# Add feature dimension if it's missing (e.g., when feats=1)
				if batch_data.dim() == 2: 
					feats = 1 # Assuming feats=1 if dim is 2, might need dynamic check
					batch_data = batch_data.unsqueeze(2) # Add feature dim: (B, W, 1)
				
				# Get feats dimension after potential reshape
				feats = batch_data.shape[2] 
...
if __name__ == '__main__':
	...
	if not args.test:
		...
		y_pred = model(testD)
		if isinstance(y_pred, tuple):  # Some models return multiple values
			y_pred = y_pred[0]  # Take the first output as prediction
		
		# Reshape predictions to match original data shape
		y_pred = y_pred[:, -1, :]  # Take the last prediction from each window
		
		# Calculate loss
		loss = torch.mean((y_pred - testO) ** 2, dim=1)  # MSE loss per timestamp
		
		# Plot results
		plotter(f'{args.model}_{args.dataset}', testO.cpu().numpy(), y_pred.cpu().detach().numpy(), loss.cpu().detach().numpy()[:, None], labels_np)
